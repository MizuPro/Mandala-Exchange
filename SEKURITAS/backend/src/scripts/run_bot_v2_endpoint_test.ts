...
async function runValidation() {
  ...
  try {
    ...
    console.log("2. Testing POST /bot/internal/tokens");
    ...
    console.log("3. Testing POST /bot/internal/genesis");
    ...
    console.log("4. Testing POST /bot/internal/portfolio-snapshot");
    ...
    // Test Place Order via /bot/orders
    ...
    // Idempotency check via /bot/orders
    ...
    // Lookup via /bot/orders/by-client-id/:clientOrderId
    const lookupOrder = await app.inject({
      method: "GET", url: `/bot/orders/by-client-id/${encodeURIComponent(stableClientOrderId)}`,
    ...
    if (!terminalOrderStatuses.has(String(placedBody.status || "").toLowerCase())) {
      const openSnapshot = await app.inject({
        method: "POST", url: "/bot/internal/portfolio-snapshot",
      ...
      // Cancel order via /bot/orders/:id
      const cancelOrder = await app.inject({
        method: "DELETE", url: `/bot/orders/${placedBody.id}`,
        headers: { authorization: `Bearer ${primaryToken}` }
      });
    ...
    const partialLookup = await app.inject({
      method: "GET", url: `/bot/orders/by-client-id/${encodeURIComponent(partialSellClientId)}`,
      headers: { authorization: `Bearer ${primaryToken}` }
    });
    ...
    const cancelPartial = await app.inject({
      method: "DELETE", url: `/bot/orders/${partialSellBody.id}`,
      headers: { authorization: `Bearer ${primaryToken}` }
    });
    ...
    console.log("6. Testing normative IPO reserve/cancel/allocation/refund/listing via /bot/ipo");
    ...
    // Subscribe IPO via /bot/ipo/:id/subscribe
    const cancelledIpo = await createIpo();
    const cancelledSubscription = await app.inject({
      method: "POST", url: `/bot/ipo/${cancelledIpo.id}/subscribe`,
    ...
    // Cancel IPO via /bot/ipo/:id/subscriptions/:subId/cancel
    const cancelResponse = await app.inject({
      method: "POST", url: `/bot/ipo/${cancelledIpo.id}/subscriptions/${cancelledBody.subscription_id}/cancel`,
    ...
    const allocatedSubscription = await app.inject({
      method: "POST", url: `/bot/ipo/${allocatedIpo.id}/subscribe`,
      headers: { authorization: `Bearer ${botJwt}`, "idempotency-key": crypto.randomUUID() },
      payload: { requested_shares: 1000 }
    });
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const failedGenesis = await app.inject({
        method: "POST", url: "/bot/internal/genesis",
    ...
    console.log("ALL BOT V2 ENDPOINT TESTS PASSED SUCCESSFULLY");
  } catch (err) {
